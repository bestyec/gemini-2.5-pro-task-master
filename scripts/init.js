...
function createProjectStructure(projectName, projectDescription, projectVersion, authorName, skipInstall) {
  ...
  console.log(boxen(
    ...
    chalk.white('1. ') + chalk.yellow('Rename .env.example to .env and add your GEMINI_API_KEY and PERPLEXITY_API_KEY') + '\n' +
  ...
  log('info', 'Creating initial .env file...');
  try {
    if (!fs.existsSync('.env')) {
      let envContent = `# API Keys (Required)
GEMINI_API_KEY=your_gemini_api_key_here # Get from Google AI Studio or GCP
PERPLEXITY_API_KEY=your_perplexity_api_key_here # Optional, for research features

# Model Configuration
MODEL=gemini-2.5-pro-exp-03-25 # Default model for new projects
PERPLEXITY_MODEL=sonar-pro # Default Perplexity model for new projects
MAX_TOKENS=8192
TEMPERATURE=0.5
...
`;
      ...
      log('success', 'Created .env file with default settings.');
